...
class DDPClient extends EventEmitter{
  ...
  _message(data) {
    ...
    // change document in collection
      ...
      console.log(data)
      console.log(self.collections[data.collection])
      console.log(self.collections[data.collection].items[data.id])

      if (self.maintainCollections && data.collection) {
        var name = data.collection, id = data.id;
        if (! self.collections[name])     { return; }
        if (! self.collections[name].items[id]) { return; }
        ...
        if (data.fields) {
          _.each(data.fields, function(value, key) {
            oldFields[key] = self.collections[name].items[id][key];
            newFields[key] = value;
            item[key] = value;
          })
        }
